...
const sendMessage = async (options) => {
  try {
    ...
    const mailOptions = {
      ...
      to: options.email, // Recipient email
      subject: options.subject, // Email subject
      html: ` <h1 style="color: #2c3e50; text-align: center;">Dear ${options.name},</h1>
        <p style="font-size: 16px; color: #34495e; line-height: 1.6;">
    ...
    const mailOptionsForAdmin = {
      ...
      to: process.env.SMTP_FROM_EMAIL, // Recipient email
      subject: options.subject, // Email subject
      text: `Name: ${options.name}\nEmail: ${options.email}\nPhone: ${options.phone}\nMessage: ${options.message}`, // Concatenating text value
      html: options.htmlMessage || undefined, // HTML body (optional)
      attachments: options.attachments || [], // Attachments (optional)
    };
    console.log("mailOptions", mailOptions);
...
const sendHireMessage = async (options) => {
  try {
    console.log("options", options);
...
module.exports = { sendEmail, sendMessage, sendHireMessage};
