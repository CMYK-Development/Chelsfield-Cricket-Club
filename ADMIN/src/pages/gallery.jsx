...
import { addGalleryImageUrl, getGalleryImageUrl, serverUrl } from '../utils/apiRoutes';
   ...
   // Load images from local storage on component mount
    ...
    const confirmAddImages = async () => {
        try {
            ...
            const response = await axios.post(addGalleryImageUrl, formData);
        ...
        try {
            ...
            await Promise.all(
                selectedImages.map(async (img) => {
                    await axios.delete(`https://backend-chelsfield.ironstepsoftware.com/api/v1/gallery/${img.id}`);
                })
            );
    
            // Update the state to remove deleted images
            setImages((prevImages) => prevImages.filter((img) => !img.selected));
    
            console.log("Selected images deleted successfully.");
        } catch (error) {
            console.error("Error deleting selected images:", error.response ? error.response.data : error.message);
            alert("Failed to delete selected images. Please try again.");
        }
    };
    // console.log("addGalleryImageUrl",addGalleryImageUrl)

    return (
        <div className="p-6">
            <h1 className="text-2xl font-bold mb-4">Gallery Image Manager</h1>
            ...
            <div className="relative p-4 rounded-md shadow-md mb-6 border">
                {selectedImages.length > 0 ? (
                    ...
                    <p className="text-center text-gray-600">No images selected for the Gallery.</p>
                )}
            </div>
